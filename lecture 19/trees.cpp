...
mypair heightdaimeter(node * root){
	...
	mypair p1=heightdaimeter(root->left);
	mypair p2=heightdaimeter(root->right);

	p.hei=max(p1.hei,p2.hei)+1;
	p.dai=max(p1.hei+p2.hei+1,max(p1.dai,p2.dai));	
	return p;
}
...
int main()
{
	...
	// mypair ans=heightdaimeter(root);
	// cout<<ans.hei<<endl<<ans.dai<<endl;
